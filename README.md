...
Repeat-rich candidates can have large `secondary_events` or `molecules` JSON
fields. All breakpoint-table readers use the largest platform-supported CSV
field limit instead of Python's default 131,072-character limit. Existing TSV
quoting is preserved, so cached tables remain readable without conversion or
truncating alternative placements.

If a run stopped with `_csv.Error: field larger than field limit (131072)`,
resume the original command with `-resume` after updating the scripts. Completed
